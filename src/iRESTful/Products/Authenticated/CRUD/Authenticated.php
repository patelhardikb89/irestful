...
function authenticated_crud_validateBaseUrl(string $test) {
    if (filter_var($test, FILTER_VALIDATE_URL) === false) {
        throw new \Exception('The baseUrl ('.$test.') is invalid.');
    }
};

function authenticated_crud_validateUri(string $uri) {
...
function authenticated_crud_validateHashedPassword(string $hashedPassword) {
...
function authenticated_crud_validateKeyname(string $keyname) {
